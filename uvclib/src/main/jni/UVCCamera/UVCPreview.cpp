...
int UVCPreview::setPreviewSize(int width, int height, int frameType, int fps,
    ...
    requestBandwidth = (bandwidth > 0.0f && bandwidth <= 1.0f) ? bandwidth : 1.0f;
...
int UVCPreview::startPreview() {
    ...
    if (!isRunning()) {
        mIsRunning = true;
        // pthread_t values are opaque handles on Android.  Reusing a value that was
        // already joined makes recent bionic versions abort the whole process instead
        // of merely returning ESRCH.  A failed UVC stream start does not create the
        // capture thread, so clear both handles before every new preview generation.
        preview_thread = 0;
        capture_thread = 0;
        pthread_mutex_lock(&preview_mutex);
        {
...
int UVCPreview::stopPreview() {
    ...
    if (LIKELY(b)) {
        ...
        pthread_cond_signal(&preview_sync);
        pthread_cond_signal(&capture_sync);
        // Join the producer first.  It is responsible for creating capture_thread;
        // waiting for it removes the race where stopPreview observed the stale handle
        // from a previous generation just before the new thread was created.
        const pthread_t preview_to_join = preview_thread;
        preview_thread = 0;
        if (preview_to_join && pthread_join(preview_to_join, NULL) != EXIT_SUCCESS) {
            LOGW("UVCPreview::terminate preview thread: pthread_join failed");
        }
        const pthread_t capture_to_join = capture_thread;
        capture_thread = 0;
        if (capture_to_join && pthread_join(capture_to_join, NULL) != EXIT_SUCCESS) {
...
//    time_t c_start, c_end;
    ...
    int result = uvc_start_streaming_bandwidth(
    ...
    if (LIKELY(!result)) {
        clearPreviewFrame();
        capture_thread = 0;
        const int capture_result = pthread_create(
                &capture_thread, NULL, capture_thread_func, (void *) this);
        if (capture_result == EXIT_SUCCESS) {
            pthread_setname_np(capture_thread, "capture_thread");
        } else {
            capture_thread = 0;
            LOGE("could not create capture thread: %d", capture_result);
            mIsRunning = false;
        }
...
//                    c_start = clock();
                    // Use libuvc's libjpeg decoder for UVC MJPEG compatibility. Some
                    // devices (including DJI Osmo Pocket 4) omit the standard Huffman
                    // tables. uvc_mjpeg2rgbx inserts those tables; TurboJPEG rejects the
                    // same frames and leaves the preview permanently black.
...
//                    LOGI("uvc_mjpeg2yuyv time: %f", (double) (c_end - c_start) / CLOCKS_PER_SEC);
                    if (LIKELY(!result)) {
                        ...
                        LOGW("MJPEG decode failed: %d, bytes=%zu", result,
