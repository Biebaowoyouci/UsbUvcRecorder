...
static jint nativeSetPreviewSize(JNIEnv *env, jobject thiz,
                                 ...
                                 jint frameType, jint fps, jfloat bandwidth) {

    ENTER();
    UVCCamera *camera = reinterpret_cast<UVCCamera *>(id_camera);
    if (LIKELY(camera)) {
        return camera->setPreviewSize(width, height, frameType, fps, bandwidth);
    }
    RETURN(JNI_ERR, jint);
}
...
static JNINativeMethod methods[] = {
        ...
        {"nativeSetPreviewSize",      "(JIIIIF)I",                                 (void *) nativeSetPreviewSize},
