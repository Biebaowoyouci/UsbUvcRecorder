...
public class RendererHolder extends EGLTask implements IRendererHolder {
    ...
    /** Sets a transform for one output surface without changing the other outputs. */
    public void setSlaveMvpMatrix(final int id, final float[] matrix) {
        synchronized (mSlaveSurfaces) {
            final RendererSurface slaveSurface = mSlaveSurfaces.get(id);
            if (slaveSurface == null) {
                throw new IllegalStateException("Slave surface is not attached: " + id);
            }
            slaveSurface.setMvpMatrix(matrix);
        }
    }
    ...
    protected class RendererHandler extends Handler {
        ...
        private void handleInit() {
            ...
            // The public isRunning() guard is used by clients before adding an encoder
            // surface.  This vendored implementation never updated the flag, so a valid
            // renderer was permanently reported as stopped and every recording attempt
            // was rolled back immediately after MediaCodec.start().
            isRunning = true;
        }
