...
class RendererSurface {
    ...
    private boolean mUseCustomMvpMatrix;
    ...
    void setMvpMatrix(final float[] matrix) {
        if (matrix == null || matrix.length < 16) {
            throw new IllegalArgumentException("MVP matrix must contain 16 values");
        }
        System.arraycopy(matrix, 0, mMvpMatrix, 0, 16);
        mUseCustomMvpMatrix = true;
    }
